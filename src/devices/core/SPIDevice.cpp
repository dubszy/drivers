...
SPIDevice::SPIDevice(string name, int channel, uint32_t speed, uint8_t bitsPerWord)
  : Device(name) { // NOLINT do not std::move 'name'
    ...
    if ((fd_ = open(channel_ == 0 ? device0 : device1, O_RDWR)) < 0) {
        log->error("Unable to open SPI device: %s", strerror(errno));
    } else {
        if (ioctl(fd_, SPI_IOC_WR_MODE, &mode) < 0) {
            log->error("Could not change mode for SPI device (%s)", strerror(errno));
        }
        if (ioctl(fd_, SPI_IOC_WR_BITS_PER_WORD, &bitsPerWord_) < 0) {
            log->error("Could not set bits per word for SPI device (%s)", strerror(errno));
        }
        if (ioctl(fd_, SPI_IOC_WR_MAX_SPEED_HZ, &speed_) < 0) {
            log->error("Could not set speed for SPI device (%s)", strerror(errno));
        }
    }
}
...
string SPIDevice::toString() {
    stringstream sstream;
    sstream << "SPIDevice: { name: " << name_
            << ", channel: " << channel_
            << ", speed: " << speed_ << "Hz"
            << ", Bits-per-Word: " << (uint32_t)bitsPerWord_ // uint8_t is scanned verbatim
            << " }";
    return sstream.str();
}
