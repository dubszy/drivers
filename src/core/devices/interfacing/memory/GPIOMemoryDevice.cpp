...
GPIOEdgeDetect GPIODevice::GPIODeviceImpl::getEdgeDetect() const {
    GPIOEdgeDetect edge_detect;

    edge_detect |= (gpio_is_event_detect_rising(gpio_num_))        ? GPIOEdgeDetectRising       : 0;
    edge_detect |= (gpio_is_event_detect_falling(gpio_num_))       ? GPIOEdgeDetectFalling      : 0;
    edge_detect |= (gpio_is_event_detect_high(gpio_num_))          ? GPIOEdgeDetectHigh         : 0;
    edge_detect |= (gpio_is_event_detect_low(gpio_num_))           ? GPIOEdgeDetectLow          : 0;
    edge_detect |= (gpio_is_event_detect_async_rising(gpio_num_))  ? GPIOEdgeDetectAsyncRising  : 0;
    edge_detect |= (gpio_is_event_detect_async_falling(gpio_num_)) ? GPIOEdgeDetectAsyncFalling : 0;
